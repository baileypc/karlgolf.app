...
function sendPasswordResetEmail($email, $token) {
    // Get the current domain (works for both http and https)
    $protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'] ?? 'karlsgolf.app';
...
function buildPasswordResetEmail($resetUrl, $expiresIn) {
    // Get the current domain for footer link
    $protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'] ?? 'karlsgolf.app';
    $html = <<<HTML
<!DOCTYPE html>
    ...
    <div class="email-container">
        ...
        <div class="footer">
            ...
            <p style="margin-top: 15px;">
                <a href="{$protocol}://{$host}">Visit karlsgolf.app</a>
            </p>
        </div>
    </div>
</body>
