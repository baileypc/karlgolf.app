...
function mergeRound($existingRound, $newRoundData) {
    ...
    // If new holes array has fewer holes than existing, this indicates a DELETE operation
    // In this case, REPLACE the entire holes array instead of merging
    // This ensures deleted holes are permanently removed
    if (count($newHoles) < count($existingHoles)) {
        // REPLACE mode: Use new holes array as-is (deleted holes are gone)
        $mergeResult = [
            'holes' => $newHoles,
            'added' => 0,
            'updated' => count($newHoles) // All holes were "updated" (replaced)
        ];
    } else {
        // MERGE mode: Merge new holes into existing
    ...
    if (!$isReplace && $mergeResult['added'] === 0 && $mergeResult['updated'] === 0) {
        return [
            'success' => false,
            'round' => null,
            'added' => 0,
